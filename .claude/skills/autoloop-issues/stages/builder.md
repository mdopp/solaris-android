...
1. Pick the first `status:"planned"` unit from `queue[]`. Set `in_progress`.
...
3. Push the branch; open a PR into `main` titled for the batch, body listing the issues (`Closes #a, #b…`).
   - **If any unit is `security:true`** → open the PR as **draft**, add each to `review[]` with the flag, mirror `autoloop:needs-review`, and **stop** (never auto-merge signing/keystore/token-contract changes).
...
5. Set `verify_state = {sha, status:"owed"}`. For each `gate:"verify"` unit, add to
   `device_test[]` `{issue, pr, apk:"app/build/outputs/apk/release/app-release.apk", what_to_check, since}` — the human sideloads + confirms. Move units to `completed[]`.
